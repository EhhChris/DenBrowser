...
**Currently tracking Firefox ESR 153.x** (most recently built against 153.1.0esr;
...
| Developer tools (incl. CLI debug flags) | 008, 015 | `DisableDeveloperTools`, `BlockAboutConfig`, `BlockAboutProfiles`, `BlockAboutSupport`, `BlockAboutAddons` | `devtools.policy.disabled`, `devtools.chrome.enabled`, `devtools.debugger.remote-enabled`, `devtools.debugger.prompt-connection` |
| Branding | 009 | — | — |
| Telemetry / diagnostics / remote rollouts | 010 | `DisableTelemetry`, `DisableFirefoxStudies`, `DisableRemoteImprovements`, `DisableFeedbackCommands` | `datareporting.policy.dataSubmissionEnabled`, `datareporting.healthreport.uploadEnabled`, `toolkit.telemetry.enabled`, `toolkit.telemetry.unified`, `app.normandy.enabled`, `app.shield.optoutstudies.enabled`, `nimbus.rollouts.enabled` |
...
| Camera / Microphone / Location / Notifications / VR | — | `Permissions: Camera/Microphone/Location/Notifications/VirtualReality {BlockNewRequests: true, Locked: true}`, `Autoplay: block-audio-video` | `geo.enabled`, `media.peerconnection.enabled`, `media.navigator.enabled` |
| Cookies — in-memory, no third-party | — | `Cookies: reject-foreign + Locked`, `SanitizeOnShutdown.Cookies` | (via policy) |
| DRM / EME | — | `EncryptedMediaExtensions {Enabled: false, Locked: true}` | — |
| Password manager / form autofill | — | `PasswordManagerEnabled: false`, `DisableMasterPasswordCreation`, `DisablePasswordReveal`, `DisableProfileImport`, `AutofillAddressEnabled: false`, `AutofillCreditCardEnabled: false` | `signon.rememberSignons`, `signon.autofillForms`, `signon.generation.enabled`, `browser.contextual-password-manager.enabled`, `pref.privacy.disable_button.view_passwords`, `extensions.formautofill.addresses.*`, `extensions.formautofill.creditCards.*` |
| Built-in AI / agent actions | — | `AIControls: {Default: {Value: blocked, Locked: true}}` | `browser.ai.control.*`, `browser.ml.chat.*`, `browser.ml.linkPreview.enabled`, `browser.ml.pageAssist.enabled`, `browser.smartwindow.enabled`, `browser.tabs.groups.smart.userEnabled` |
| Bookmarks | 019 | `NoDefaultBookmarks`, `DisplayBookmarksToolbar: never` | `browser.urlbar.suggest.bookmark`, `browser.urlbar.shortcuts.bookmarks`, `browser.toolbars.bookmarks.visibility`, `browser.bookmarks.max_backups`, `browser.bookmarks.autoExportHTML` |
| First-run / new-tab / stories / messaging | — | `OverrideFirstRunPage`, `OverridePostUpdatePage`, `NoDefaultBookmarks`, `FirefoxHome`, `UserMessaging` | — |
| App + system-addon updates | — | `DisableAppUpdate`, `DisableSystemAddonUpdate` | `extensions.update.enabled`, `extensions.systemAddon.update.enabled`, `app.update.url.manual`, `app.update.url.details` |
| HTTPS-only enforcement | — | `HttpsOnlyMode: force_enabled` | — |
| Translation / search-suggest / DoH | — | `AIControls.Default: blocked`, `SearchSuggestEnabled: false`, `DNSOverHTTPS {Enabled: false, Locked: true}`, `NetworkPrediction: false` | `network.dns.disablePrefetch`, `network.dns.disablePrefetchFromHTTPS`, `network.prefetch-next`, `network.predictor.enable-hover-on-ssl`, `network.http.speculative-parallel-limit`, `browser.urlbar.suggest.history`, `browser.urlbar.suggest.bookmark` |
...
| External protocol handlers (mailto/tel/sms/…) | — | — | `network.protocol-handler.external-default`, `network.protocol-handler.external.mailto`, `network.protocol-handler.external.tel`, `network.protocol-handler.external.sms` |
| Share to OS share pane / Mozilla service | — | — | `browser.menu.share_url.allow`, `browser.toolbars.share-button.enabled`, `browser.contentsharing.enabled` |
...
| 003 | `restrict-clipboard` | Block clipboard read/write and drag-to-copy everywhere while retaining trusted internal tab transport for reordering; blocked drags complete Firefox's normal session cleanup, and native tab payloads contain no page URL. Includes an optional in-process clipboard for `clipboard_sites` and non-modal feedback for blocked user copy attempts. |
